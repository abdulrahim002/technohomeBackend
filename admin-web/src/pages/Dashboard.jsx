    ...
    <div className="flex justify-between items-start">
      ...
      {trend && (
        <div className="flex flex-col items-end ">
          <span className="text-emerald-600 text-[10px] font-black bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-100">
            +{trend}%
          </span>
        </div>
      )}
...
const Dashboard = () => {
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-[60vh] text-center px-6">
      ...
      <h2 className="text-2xl font-black text-slate-900 mb-3">عذراً، حدث خطأ</h2>
      <p className="text-slate-400 font-bold text-sm mb-8">{error}</p>
      <button 
        onClick={() => window.location.reload()}
        className="bg-indigo-600 hover:bg-indigo-500 text-white font-black px-8 py-3 rounded-2xl shadow-lg shadow-indigo-600/20 transition-all"
      >
        إعادة تحميل الصفحة
      </button>
    </div>
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-4xl font-black text-slate-900 mb-2 font-outfit tracking-tight">مرحباً بك مجدداً</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <DashboardCard 
          title="أرباح المنصة" 
          value={`${stats?.financials?.totalRevenue || 0} د.ل`} 
          icon={<TrendingUp size={24} />} 
          color="bg-amber-500"
        />
        <DashboardCard 
          title="سيولة الفنيين" 
          value={`${stats?.financials?.systemLiquidity || 0} د.ل`} 
          icon={<Wallet size={24} />} 
          color="bg-purple-500"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 glass-card p-10">
            <div className="flex justify-between items-center mb-10">
               <h3 className="text-xl font-black text-slate-900 font-outfit">النشاط الأخير</h3>
            ...
            <div className="space-y-4">
               {[1, 2, 3].map(i => (
                 <div key={i} className="flex items-center justify-between p-5 bg-white hover:bg-slate-50 rounded-[24px] border border-slate-100 transition-all group">
                    <div className="flex items-center gap-5">
                       <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 group-hover:border-indigo-200 transition-all">
                          <Clock className="text-slate-400 group-hover:text-indigo-600 transition-colors" size={24} />
                       </div>
                       <div>
                          <p className="text-slate-900 font-bold text-base">عملية تحديث نظام</p>
         ...
         <div className="glass-card p-10 flex flex-col">
             <h3 className="text-xl font-black text-slate-900 mb-10 font-outfit">توزيع المنصة</h3>
             <div className="space-y-8 flex-1">
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-4">
                      <div className="w-2.5 h-2.5 bg-blue-500 rounded-full shadow-sm shadow-blue-500/50" />
                      <span className="text-slate-500 font-bold text-sm">العلامات التجارية</span>
                   </div>
                   <span className="text-slate-900 font-black text-lg">{stats?.system?.brands || 0}</span>
                </div>
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-4">
                      <div className="w-2.5 h-2.5 bg-indigo-500 rounded-full shadow-sm shadow-indigo-500/50" />
                      <span className="text-slate-500 font-bold text-sm">أنواع الأجهزة</span>
                   </div>
                   <span className="text-slate-900 font-black text-lg">{stats?.system?.applianceTypes || 0}</span>
                </div>
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-4">
                      <div className="w-2.5 h-2.5 bg-amber-500 rounded-full shadow-sm shadow-amber-500/50" />
                      <span className="text-slate-500 font-bold text-sm">طلبات معلقة</span>
                   </div>
                   <span className="text-amber-500 font-black text-lg">{stats?.serviceRequests?.pending || 0}</span>
                </div>
             </div>
             
             <div className="mt-10 p-4 bg-slate-50 rounded-2xl border border-white/5">
