...
const Technicians = () => {
  ...
  const fetchTechnicians = async () => {
    setLoading(true);
    try {
      const endpoint = filter === 'pending' ? '/admin/technicians/pending' : '/admin/users?role=technician';
      const response = await api.get(endpoint);
      
      let normalizedData = [];
      if (filter === 'pending') {
        normalizedData = (response.data.data.technicians || []).map(t => ({
          _id: t._id,
          userId: t.user?._id,
        ...
        normalizedData = (response.data.data.users || []).map(u => ({
          _id: u._id,
          userId: u._id,
          firstName: u.firstName || 'غير معروف',
          lastName: u.lastName || '',
          phone: u.phone || 'غير متوفر',
          location: u.city?.nameAr || 'غير محدد',
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-right">
        <div className="flex-1">
          <h2 className="text-3xl font-black text-white mb-2 font-outfit tracking-tight">إدارة الفنيين</h2>
           ...
           {['pending', 'verified'].map(type => (
              <button
                ...
                className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all
                  ${filter === type ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-500 hover:text-white'}`}
              >
                    ...
                    <tr key={tech._id} className="hover:bg-white/[0.01] transition-all group">
                       ...
                       <td className="px-8 py-6">
                          <button 
                             ...
                             className="p-2.5 bg-blue-600/10 text-blue-500 rounded-xl hover:bg-blue-600 hover:text-white transition-all scale-0 group-hover:scale-100 origin-left"
                          >
                             <Eye size={18} />
                          </button>
                       </td>
                       ...
                       <td className="px-8 py-6 text-right">
                          <div className="flex items-center justify-end gap-3">
                             <div className="text-right">
                                <p className="text-white font-black text-sm">{tech.firstName} {tech.lastName}</p>
                                <p className="text-slate-500 text-[10px] font-bold mt-0.5">{tech.phone}</p>
                             </div>
                             <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center border border-white/5 group-hover:border-blue-500/30 transition-all overflow-hidden">
                                {tech.profileImage ? (
        ...
        {showModal && selectedTech && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
             ...
             <motion.div 
                ...
                className="relative w-full max-w-4xl glass p-10 rounded-[40px] shadow-2xl border border-white/10 max-h-[90vh] overflow-y-auto"
             >
                <div className="flex justify-between items-center mb-10">
                   <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-2xl font-black text-white font-outfit">تفاصيل ملف الفني</h3>
                   </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                   ...
                   <div className="md:col-span-2 space-y-8">
                      <div className="space-y-4">
                         <h4 className="text-white font-black text-lg text-right pr-2">التخصصات والماركات</h4>
                   ...
                   <div className="space-y-6">
                      <div className="glass-card p-8 flex flex-col items-center text-center">
                         <div className="w-24 h-24 bg-slate-900 rounded-[32px] border-2 border-white/5 mb-6 flex items-center justify-center overflow-hidden">
                         ...
                         <h4 className="text-2xl font-black text-white mb-1 font-outfit">
                         ...
                         <div className="w-full space-y-4 pt-6 border-t border-white/5">
                            <div className="flex justify-between items-center px-2">
                               ...
                               <span className="text-white font-black text-sm">{selectedTech.phone}</span>
                            </div>
                            <div className="flex justify-between items-center px-2">
                               <span className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">City</span>
                               <span className="text-white font-black text-sm">{selectedTech.location}</span>
                            </div>
                         </div>
                      </div>
