...
[in_progress]     --> قيد الصيانة والعمل الفعلي
   │
   ▼
[completed]       --> إتمام الصيانة بنجاح (تح�## 6. ذكاء اصطناعي تفاعلي، تشخيص صوتي وحماية الحصة اليومية (Gemini AI Multi-modal, Voice Notes & Quotas)

تم ربط النظام بمحرك متقدم للذكاء الاصطناعي مع قيود أمنية لمنع إهدار الحصص المجانية وتسهيل تجربة الاستخدام من خلال استقبال مدخلات صوتية مباشرة:

### 🎤 التشخيص الصوتي المبتكر (Voice Note Diagnosis Service):
يدعم التطبيق استقبال مقاطع صوتية (Voice Notes) من المستخدم لشرح المشكلة بدلاً من الكتابة أو كإضافة لها، ويتم معالجتها كالتالي:
1. **الرفع المؤقت الآمن للباكيند**: يتم استقبال الملف الصوتي عبر Middleware مخصص (`uploadVoice`) يدعم الامتدادات الشائعة (`m4a, wav, mp3, amr, caf, ogg`) بحجم أقصى **10 ميجابايت**.
2. **تحليل الصوت المتعدد الوسائط (Multi-modal Audio Parsing)**: يتم قراءة ملف الصوت وتحويله إلى Buffer بصيغة base64، ثم يمرر مباشرة كجزء من محتويات الطلب (`inlineData`) لنموذج Gemini (مثل `gemini-2.5-flash` أو `gemini-2.5-flash-lite`) الذي يدعم الصوت أصلياً، دون الحاجة لرفع دائم على خوادم خارجية.
3. **توليد استجابة JSON موحدة**: يتم توجيه النموذج للاستماع بدقة وإرجاع المشكلة والحل باللغة العربية الفصحى وبنفس البنية المهيكلة المعتادة للـ JSON: `{ diagnosis, steps }`.
4. **التنظيف التلقائي للملفات المؤقتة**: بمجرد استلام رد الـ AI، يقوم الباكيند فوراً بحذف الملف الصوتي من السيرفر (`fs.unlink`) في كتلة `finally` لضمان عدم تراكم الملفات وتوفير مساحات القرص.
5. **معالجة وحجز فني بدون كراش**:
   - عند اختيار "اكتفيت بالتقرير" أو "حجز فني"، إذا كان الوصف النصي الأصلي للعميل فارغاً (بسبب اعتماده على الصوت فقط)، يقوم التطبيق برمجياً بتعبئة حقل الوصف (`problemDescription`) بنتيجة التشخيص الراجعة من Gemini (`aiDiagnosis.diagnosis`) كبديل ذكي يضمن تلبية متطلبات قاعدة البيانات.

### 📱 تنظيم وتنسيق عرض البيانات للفني والعميل (UX Data Mapping):
لمنع التكرار البصري وتحسين تجربة القراءة:
- **في تطبيق العميل**: يُعرض في كارت الوصف العلوي عنوان مختصر فقط: *"طلب فحص [نوع الجهاز] عبر التسجيل الصوتي"*، بينما يُترك التقرير الكامل التفصيلي والخطوات ليظهرا في كارت *"تفاصيل التشخيص الذكي"* بالأسفل.
- **في تطبيق الفني**: 
  * يُعرض العنوان المختصر في قائمة الطلبات (Orders List) لسهولة القراءة والفرز السريع.
  * في شاشة تفاصيل المهمة، يُعرض كارت التقرير الذكي الكامل للفني **حتى لو كانت حالة المهمة معلقة (Pending)** ليتمكن من قراءة وفهم العطل قبل قبول الطلب أو التحرك للعميل.
...
1. **الفحص الفوري المجرد (بدون حفظ الطلب):** يخضع السيرفر لمهلة زمنية صارمة قدرها **20 ثانية**؛ إذا لم يستجب موديل `gemini-2.5-flash` أو `gemini-2.5-flash-lite` خلالها، ينقطع الاتصال ويعاد رد فني بديل ذكي وجاهز يحافظ على سرعة استجابة التطبيق أمام العميل.
...
* يتم حساب إعادة التعبئة تلقائياً بمجرد مرور 24 ساعة على آخر عملية تصفير وإعادة تعيين الرصيد إلى 5 مجدداً. **بعد التحول لحالة القبول (`accepted`):**
